...
# Import validate_extraction from kebab-case module using importlib
_validate = importlib.import_module("my_llm_wiki.validate-schema")
validate_extraction = _validate.validate_extraction


def build_from_json(extraction: dict) -> nx.Graph:
    """Build a NetworkX graph from a single extraction dict.

    Validates the extraction first, warns on real schema errors (not dangling edges).
    Dangling edges (stdlib/external imports) are silently skipped — expected behavior.
    """
    errors = validate_extraction(extraction)
    # Dangling edges (stdlib/external imports) are expected - only warn about real schema errors.
    real_errors = [e for e in errors if "does not match any node id" not in e]
    if real_errors:
        print(f"[wiki] Extraction warning ({len(real_errors)} issues): {real_errors[0]}", file=sys.stderr)
    G = nx.Graph()
